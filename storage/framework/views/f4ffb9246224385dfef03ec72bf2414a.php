...
<?php $__env->startSection('content'); ?>
    ...
    <div class="mt-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <?php
                $availableRoles = $employees
                    ->map(fn ($employee) => $employee->roles->first())
                    ->filter()
                    ->unique('name')
                    ->sortBy(fn ($role) => $roleLabels[$role->name] ?? $role->name);
            ?>

            <?php if($availableRoles->isNotEmpty()): ?>
                <select
                ...
                >
                    ...
                    <?php $__currentLoopData = $availableRoles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($role->name); ?>"><?php echo e($roleLabels[$role->name] ?? $role->name); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            <?php endif; ?>
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('users_create')): ?>
            ...
            >
                <i class="fa-solid fa-plus" aria-hidden="true"></i> Nuevo empleado
            </a>
        <?php endif; ?>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800" role="grid">
                ...
                <tr class="text-left text-xs font-semibold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">
                    ...
                    <th class="hidden px-4 py-3 sm:px-6 lg:table-cell">Teléfono</th>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr
                        data-employee-row
                        data-employee-search="<?php echo e(strtolower(trim($employee->full_name . ' ' . $employee->email . ' ' . ($employee->DUI ?? '') . ' ' . ($employee->phone ?? '') . ' ' . ($roleLabels[$employee->roles->first()?->name] ?? $employee->roles->first()?->name ?? '')))); ?>"
                        data-employee-role="<?php echo e($employee->roles->first()?->name); ?>"
                        data-employee-active="<?php echo e($employee->is_active ? '1' : '0'); ?>"
...
<?php $__env->startSection('scripts'); ?>
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const filterRole = document.getElementById('employee-filter-role');
            const filterStatus = document.getElementById('employee-filter-status');
            ...
            function applyFilters() {
                ...
                rows.forEach((row) => {
                    const matchesSearch = row.dataset.employeeSearch.includes(term);
                    const matchesRole = !roleFilter || row.dataset.employeeRole === roleFilter;
                    const matchesStatus = !statusFilter || row.dataset.employeeActive === statusFilter;
