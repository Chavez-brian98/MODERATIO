<?php $__env->startSection('title', 'Bitácora · Registro #' . $log->id); ?>
            ...
            <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <dl class="divide-y divide-neutral-100 rounded-xl border border-neutral-100 dark:divide-neutral-800 dark:border-neutral-800">
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        ...
                        <dd>
                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold <?php echo e($actionBadges[$log->action] ?? 'bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400'); ?>">
                    ...
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        ...
                        <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white"><?php echo e($tableLabels[$log->affected_table] ?? $log->affected_table); ?></dd>
                    </div>
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">ID del registro</dt>
                        <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white"><?php echo e($log->record_id ? '#' . $log->record_id : '—'); ?></dd>
                    </div>
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Fecha y hora</dt>
                        <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white"><?php echo e($log->created_at->format('d/m/Y \a \l\a\s H:i:s')); ?></dd>
                    </div>
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Dirección IP</dt>
                        <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white"><?php echo e($log->source_ip ?? '—'); ?></dd>
                    </div>
                </dl>
            </div>

            
            <?php if($changeKeys->isNotEmpty()): ?>
                <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                    <div class="mb-6 flex items-center gap-3 border-b border-brand-100 pb-4 dark:border-neutral-700">
                    ...
                    <div class="overflow-x-auto rounded-xl border border-neutral-100 dark:border-neutral-800">
                        <table class="min-w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                                <?php $__currentLoopData = $changeKeys; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr>
                                        <td class="px-4 py-2.5 align-top font-medium whitespace-nowrap text-neutral-800 dark:text-neutral-200">
                                            <?php echo e($fieldLabels[$key] ?? $key); ?>

                                        </td>
                                        <td class="px-4 py-2.5 align-top break-words text-red-600 dark:text-red-400">
                                            <?php echo e($formatValue($oldValues[$key] ?? null)); ?>

                                        </td>
                                        <td class="px-4 py-2.5 align-top break-words text-green-700 dark:text-green-400">
                                            <?php echo e($formatValue($newValues[$key] ?? null)); ?>
            ...
            <?php if($log->details): ?>
                <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                    ...
                    <div class="rounded-xl border border-neutral-100 bg-neutral-50 p-4 dark:border-neutral-800 dark:bg-neutral-800/50">
                        <pre class="overflow-x-auto text-xs text-neutral-700 whitespace-pre-wrap break-words dark:text-neutral-300"><?php echo e(is_array($log->details) ? json_encode($log->details, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : $log->details); ?></pre>
                    </div>
                </div>
            <?php endif; ?>
        </div>
        ...
        <aside class="space-y-6">
            ...
            <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <?php if($log->user): ?>
                    <div class="flex items-center gap-3">
                        <?php if($log->user->photoUrl()): ?>
                            <img
                                src="<?php echo e($log->user->photoUrl()); ?>"
                                alt="Foto de <?php echo e($log->user->full_name); ?>"
                        ...
                        <?php else: ?>
                            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white ring-2 ring-brand-200 dark:ring-neutral-700">
                                <?php echo e($log->user->initials()); ?>

                            </span>
                        <?php endif; ?>
                        <div class="min-w-0">
                            <p class="truncate font-medium text-neutral-900 dark:text-white"><?php echo e($log->user->full_name); ?></p>
                            <p class="truncate text-xs text-neutral-500 dark:text-neutral-400"><?php echo e($log->user->email); ?></p>
                        </div>
                    </div>
                    <?php if($responsibleRole): ?>
                        <p class="mt-4 text-xs text-neutral-500 dark:text-neutral-400">
                            ...
                            <span class="ml-1 inline-flex items-center rounded-full bg-brand-100 px-2.5 py-0.5 text-xs font-semibold text-brand-700 dark:bg-brand-900/40 dark:text-brand-400">
                                <?php echo e($responsibleRole->name); ?>
                    ...
                    <dl class="divide-y divide-neutral-100 rounded-xl border border-neutral-100 dark:divide-neutral-800 dark:border-neutral-800">
                        <div class="flex items-center justify-between gap-6 px-4 py-3">
                            ...
                            <dd class="min-w-0 text-right">
                                <p class="truncate text-sm font-medium text-neutral-900 dark:text-white"><?php echo e($affected['title']); ?></p>
                                <?php if($affected['subtitle']): ?>
                                    <p class="truncate text-xs text-neutral-500 dark:text-neutral-400"><?php echo e($affected['subtitle']); ?></p>
                                <?php endif; ?>
                            </dd>
                        </div>
                        <div class="flex items-center justify-between gap-6 px-4 py-3">
                        ...
                        <div class="flex items-center justify-between gap-6 px-4 py-3">
                            ...
                            <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white">
                                <?php echo e($tableLabels[$log->affected_table] ?? $log->affected_table); ?> · #<?php echo e($log->record_id); ?>
            ...
            <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <p class="text-sm leading-relaxed text-neutral-600 dark:text-neutral-400">
                    El usuario
                    <strong class="font-semibold text-neutral-900 dark:text-white"><?php echo e($log->user?->full_name ?? 'Sistema'); ?></strong>
                    realizó una acción de tipo
                    <strong class="font-semibold text-neutral-900 dark:text-white"><?php echo e(strtolower($actionLabels[$log->action] ?? $log->action)); ?></strong>
                    sobre <?php echo e(strtolower($tableLabels[$log->affected_table] ?? $log->affected_table)); ?>

                    <?php if($log->record_id): ?>
                        <strong class="font-semibold text-neutral-900 dark:text-white">#<?php echo e($log->record_id); ?></strong>
                    <?php endif; ?>
                    el <?php echo e($log->created_at->format('d/m/Y')); ?> a las <?php echo e($log->created_at->format('H:i:s')); ?>,
                    desde la IP <?php echo e($log->source_ip ?? 'desconocida'); ?>.
                </p>
            </div>
        </aside>
    </section>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Brian\PhpstormProjects\Glenda_Store\resources\views\modules\audit\show.blade.php ENDPATH**/ ?>