...
@section('content')
    ...
    <header class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">Nuevo rol</h1>
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Crea un rol para asignar permisos a los empleados.</p>
            </div>
        </div>
        <a
    ...
    <div class="mt-6 grid items-start gap-6 lg:grid-cols-3">
        <div class="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2">
            ...
            <form method="POST" action="{{ route('roles.store') }}" class="space-y-6 px-6 py-6 sm:px-8">
                @csrf
                ...
                <div>
                    ...
                    />
                    <p class="mt-1.5 text-xs text-neutral-400 dark:text-neutral-500">Escribe un nombre descriptivo para el rol.</p>
                    @error('name')
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="dashboard" {{ old('default_route') === 'dashboard' ? 'selected' : '' }}>Dashboard</option>
                        <option value="pos" {{ old('default_route') === 'pos' ? 'selected' : '' }}>POS (Punto de Venta)</option>
                        <option value="inventory.index" {{ old('default_route') === 'inventory.index' ? 'selected' : '' }}>Inventario</option>
                        <option value="categories.index" {{ old('default_route') === 'categories.index' ? 'selected' : '' }}>Categorías</option>
                        <option value="employees.index" {{ old('default_route') === 'employees.index' ? 'selected' : '' }}>Empleados</option>
                        <option value="roles.index" {{ old('default_route') === 'roles.index' ? 'selected' : '' }}>Roles y Permisos</option>
                        <option value="cash-register.index" {{ old('default_route') === 'cash-register.index' ? 'selected' : '' }}>Caja / Arqueo</option>
                        <option value="returns.index" {{ old('default_route') === 'returns.index' ? 'selected' : '' }}>Devoluciones</option>
                        <option value="reports.index" {{ old('default_route') === 'reports.index' ? 'selected' : '' }}>Reportes</option>
                        <option value="audit.index" {{ old('default_route') === 'audit.index' ? 'selected' : '' }}>Bitácora</option>
                        <option value="settings.index" {{ old('default_route') === 'settings.index' ? 'selected' : '' }}>Configuración</option>
                    </select>
            ...
            <div class="rounded-2xl border border-brand-200 bg-brand-50/60 p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-800/40">
                ...
                <ul class="mt-4 space-y-3 text-sm text-neutral-600 dark:text-neutral-400">
                    <li class="flex gap-2.5">
                        ...
                        Cada rol define un nivel de acceso dentro del sistema.
                    </li>
                    <li class="flex gap-2.5">
                        <i class="fa-solid fa-circle-check mt-0.5 text-brand-600 dark:text-brand-400" aria-hidden="true"></i>
                        Usa una descripción clara para identificar las funciones del rol.
                    </li>
                    <li class="flex gap-2.5">
                        <i class="fa-solid fa-circle-check mt-0.5 text-brand-600 dark:text-brand-400" aria-hidden="true"></i>
