        ...
        <header class="flex items-center justify-between gap-4 border-b border-brand-100 px-6 py-4 dark:border-neutral-800">
            <div class="flex min-w-0 items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-neutral-500 dark:text-neutral-400">Cliente #{{ $customer->id }}</p>
                </div>
            </div>
        ...
        <div class="overflow-y-auto px-6 py-5">
            <dl class="divide-y divide-neutral-100 dark:divide-neutral-800">
                <div class="flex items-start justify-between gap-6 py-3.5">
                    ...
                    <dd class="text-right text-sm text-neutral-900 dark:text-white">{{ $customer->tax_id ?? 'Sin documento' }}</dd>
                </div>

                <div class="flex items-start justify-between gap-6 py-3.5">
                    <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Teléfono</dt>
                    <dd class="text-right text-sm text-neutral-900 dark:text-white">{{ $customer->phone ?? 'Sin teléfono' }}</dd>
                </div>

                <div class="flex items-start justify-between gap-6 py-3.5">
                    <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Correo</dt>
                    <dd class="text-right text-sm break-all text-neutral-900 dark:text-white">{{ $customer->email ?? 'Sin correo' }}</dd>
                </div>

                <div class="flex items-start justify-between gap-6 py-3.5">
                    <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Dirección</dt>
                    <dd class="text-right text-sm text-neutral-900 dark:text-white">{{ $customer->address ?: 'Sin dirección' }}</dd>
                </div>
