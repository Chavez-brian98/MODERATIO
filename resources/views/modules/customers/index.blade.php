        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <form method="GET" action="{{ route('customers.index') }}">
                ...
                >
                    ...
                    <option value="active" {{ $status === 'active' ? 'selected' : '' }}>Solo activos</option>
                    <option value="inactive" {{ $status === 'inactive' ? 'selected' : '' }}>Solo inactivos</option>
                </select>
            </form>
            ...
            <form method="GET" action="{{ route('customers.index') }}">
                ...
                >
                    ...
                    <option value="FREQUENT" {{ request('type') === 'FREQUENT' ? 'selected' : '' }}>Frecuente</option>
                    <option value="WHOLESALER" {{ request('type') === 'WHOLESALER' ? 'selected' : '' }}>Mayorista</option>
                </select>
            </form>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800" role="grid">
                ...
                @forelse ($customers as $customer)
                    <tr
                        data-customer-row
                        data-customer-search="{{ strtolower(trim($customer->first_name.' '.$customer->last_name.' '.$customer->tax_id.' '.$customer->phone.' '.$customer->email)) }}"
                        data-customer-type="{{ $customer->customer_type }}"
                        class="{{ $customer->is_active ? 'hover:bg-brand-50/40 dark:hover:bg-neutral-800/50' : 'bg-neutral-50 hover:bg-neutral-100/60 dark:bg-neutral-800/30 dark:hover:bg-neutral-800/60' }}"
                    >
                        <td class="whitespace-nowrap px-4 py-3 text-neutral-500 dark:text-neutral-400 sm:px-6">#{{ $customer->id }}</td>
                        <td class="whitespace-nowrap px-4 py-3 sm:px-6">
                            <p class="{{ $customer->is_active ? 'font-medium text-neutral-900 dark:text-white' : 'font-medium text-neutral-400 dark:text-neutral-500' }}">
                                {{ $customer->first_name }} {{ $customer->last_name }}
                            </p>
                            @if ($customer->email)
                                <p class="mt-0.5 max-w-xs truncate text-xs text-neutral-400 dark:text-neutral-500">{{ $customer->email }}</p>
                            @endif
                        </td>
                        <td class="hidden whitespace-nowrap px-4 py-3 text-neutral-600 dark:text-neutral-400 sm:px-6 md:table-cell">{{ $customer->tax_id ?? '—' }}</td>
                        <td class="hidden whitespace-nowrap px-4 py-3 text-neutral-600 dark:text-neutral-400 sm:px-6 md:table-cell">{{ $customer->phone ?? '—' }}</td>
                        ...
                        <td class="hidden whitespace-nowrap px-4 py-3 text-neutral-600 dark:text-neutral-400 sm:px-6 lg:table-cell">{{ $customer->sales_count }}</td>
                        <td class="whitespace-nowrap px-4 py-3 sm:px-6">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right sm:px-6">
                            <div class="flex items-center justify-end gap-1.5">
                                <button
                                    type="button"
                                    title="Ver detalle"
                                    data-view-customer="{{ route('customers.show', $customer) }}"
                                ...
                                @can('customers_edit')
                                    ...
                                    <form method="POST" action="{{ route('customers.toggle', $customer) }}" class="inline">
                                        @csrf @method('PATCH')
                                        @if ($customer->is_active)
