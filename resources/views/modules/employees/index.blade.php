        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800" role="grid">
                ...
                @forelse ($employees as $employee)
                    <tr
                        data-employee-row
                        data-employee-search="{{ strtolower(trim($employee->full_name . ' ' . $employee->email . ' ' . ($employee->DUI ?? '') . ' ' . ($employee->phone ?? '') . ' ' . ($roleLabels[$employee->roles->first()?->name] ?? $employee->roles->first()?->name ?? ''))) }}"
                        data-employee-role="{{ $employee->roles->first()?->name }}"
                    ...
                    >
                        ...
                        <td class="whitespace-nowrap px-4 py-3 sm:px-6">
                            ...
                            @if (($employee->permissions_count ?? 0) > 0)
                                <p class="mt-1 inline-flex items-center gap-1 rounded-full bg-violet-50 px-2 py-0.5 text-[11px] font-medium text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
                                    ...
                                    {{ $employee->permissions_count }} {{ $employee->permissions_count === 1 ? 'permiso propio' : 'permisos propios' }}
                                </p>
                            @endif
                        </td>
