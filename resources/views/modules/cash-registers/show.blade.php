...
@section('content')
    ...
    <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            @can('cash_registers_edit')
                <a href="{{ route('cash-register.edit', $register) }}" class="inline-flex items-center gap-2 rounded-xl border border-brand-200 bg-white px-4 py-2.5 text-sm font-medium text-neutral-700 shadow-sm transition-colors hover:bg-brand-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800" title="Editar caja (corregir errores de captura)">
        ...
        <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-900 lg:col-span-2">
            ...
            <dl class="divide-y divide-neutral-100 rounded-xl border border-neutral-100 dark:divide-neutral-800 dark:border-neutral-800">
                <div class="flex items-center justify-between gap-6 px-4 py-3">
                    ...
                    <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white">{{ $register->user->full_name }}</dd>
                </div>
                <div class="flex items-center justify-between gap-6 px-4 py-3">
                    ...
                    <dd class="text-right text-sm font-medium text-neutral-900 dark:text-white">
                        @if ($register->responsible)
                            {{ $register->responsible->full_name }}
                        @else
                ...
                <div class="flex items-center justify-between gap-6 px-4 py-3">
                    ...
                    <dd class="text-right text-sm font-medium whitespace-nowrap text-neutral-900 dark:text-white">{{ $register->created_at?->format('d/m/Y \a \l\a\s H:i') ?? '—' }}</dd>
                </div>
                <div class="flex items-center justify-between gap-6 px-4 py-3">
                    <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Última actualización</dt>
                    <dd class="text-right text-sm font-medium whitespace-nowrap text-neutral-900 dark:text-white">{{ $register->updated_at?->format('d/m/Y \a \l\a\s H:i') ?? '—' }}</dd>
                </div>
                @if ($register->closing_date)
                    <div class="flex items-center justify-between gap-6 px-4 py-3">
                        <dt class="shrink-0 text-sm text-neutral-500 dark:text-neutral-400">Fecha de cierre</dt>
                        <dd class="text-right text-sm font-medium whitespace-nowrap text-neutral-900 dark:text-white">{{ $register->closing_date->format('d/m/Y \a \l\a\s H:i') }}</dd>
                    </div>
                @endif
            </dl>
        </div>
    ...
    @if ($register->status === 'OPEN')
        <div class="mx-auto mb-8 max-w-2xl rounded-2xl border border-amber-200 bg-amber-50 p-6 shadow-sm dark:border-amber-800 dark:bg-amber-950/30">
            ...
            <form method="POST" action="{{ route('cash-register.close', $register) }}" id="close-form">
                @csrf
                @method('PATCH')
                ...
                <div class="mb-4 space-y-1.5 rounded-xl bg-white p-4 text-sm dark:bg-neutral-900">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <span class="font-medium text-neutral-800 dark:text-neutral-200">${{ number_format($register->opening_amount, 2) }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <span class="text-neutral-500 dark:text-neutral-400">+ Ventas en efectivo</span>
                        <span class="font-medium text-neutral-800 dark:text-neutral-200">${{ number_format($arqueo['cash_sales'], 2) }}</span>
                    </div>
                    <div class="flex items-center justify-between gap-4 border-t border-neutral-100 pt-1.5 dark:border-neutral-700">
                        ...
                        <span class="text-lg font-bold text-brand-600 dark:text-brand-400">${{ number_format($arqueo['theoretical_amount'], 2) }}</span>
                    </div>
                ...
                <div class="mb-5">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            value="{{ old('actual_closing_amount') }}"
                            placeholder="{{ number_format($arqueo['theoretical_amount'], 2, '.', '') }}"
                            step="0.01"
                            min="0"
                            required
    ...
    @elseif ($register->actual_closing_amount !== null)
        @php
            $closingDifference = (float) $register->difference;
            $isBalanced = abs($closingDifference) < 0.005;
        @endphp
        <div class="mx-auto mb-8 max-w-2xl rounded-2xl border p-6 shadow-sm {{ $isBalanced ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/30' : ($closingDifference > 0 ? 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30' : 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30') }}">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-4 border-b pb-4 {{ $isBalanced ? 'border-emerald-200 dark:border-emerald-800' : ($closingDifference > 0 ? 'border-blue-200 dark:border-blue-800' : 'border-red-200 dark:border-red-800') }}">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl {{ $isBalanced ? 'bg-emerald-200 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400' : ($closingDifference > 0 ? 'bg-blue-200 text-blue-700 dark:bg-blue-900/50 dark:text-blue-400' : 'bg-red-200 text-red-700 dark:bg-red-900/50 dark:text-red-400') }}">
            ...
            <dl class="divide-y divide-neutral-200/70 rounded-xl bg-white p-4 text-sm dark:divide-neutral-700 dark:bg-neutral-900">
                <div class="flex items-center justify-between gap-4 px-1 py-1.5">
                    ...
                    <dd class="font-medium text-neutral-800 dark:text-neutral-200">${{ number_format($register->theoretical_closing_amount, 2) }}</dd>
                </div>
                <div class="flex items-center justify-between gap-4 px-1 py-1.5">
                    <dt class="text-neutral-500 dark:text-neutral-400">Contado al cerrar</dt>
                    <dd class="font-medium text-neutral-800 dark:text-neutral-200">${{ number_format($register->actual_closing_amount, 2) }}</dd>
                </div>
                <div class="flex items-center justify-between gap-4 px-1 pt-2">
                    ...
                    <dd class="{{ $isBalanced ? 'text-emerald-600 dark:text-emerald-400' : ($closingDifference > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400') }} text-base font-bold">
                        {{ $isBalanced ? '$0.00' : (($closingDifference > 0 ? '+' : '-') . '$' . number_format(abs($closingDifference), 2)) }}
                    </dd>
                </div>
            </dl>

            @if (filled($register->closing_notes))
                <div class="mt-3 rounded-xl border border-neutral-200/80 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900">
                    ...
                    <p class="mt-1 whitespace-pre-line text-sm text-neutral-700 dark:text-neutral-300">{{ $register->closing_notes }}</p>
                </div>
            @endif

            @if ($register->closing_date)
                <p class="mt-3 text-xs {{ $isBalanced ? 'text-emerald-700/80 dark:text-emerald-400/80' : ($closingDifference > 0 ? 'text-blue-700/80 dark:text-blue-400/80' : 'text-red-700/80 dark:text-red-400/80') }}">
                    Cerrada el {{ $register->closing_date->format('d/m/Y \a \l\a\s H:i') }}.
                </p>
            @endif
        </div>
    @endif
                    ...
                    @forelse ($arqueo['sales'] as $sale)
                        <tr class="transition-colors hover:bg-brand-50/40 dark:hover:bg-neutral-800/40">
                            <td class="px-2 py-3">
                                ...
                                >
                                    <i class="fa-solid fa-chevron-right text-xs transition-transform" data-sale-icon="{{ $sale->id }}"></i>
                                    <span class="sr-only">Ver productos de {{ $sale->ticket_number }}</span>
                                </button>
                            </td>
                            <td class="px-4 py-3 font-medium whitespace-nowrap text-neutral-800 dark:text-neutral-200">{{ $sale->ticket_number }}</td>
                            <td class="hidden px-4 py-3 whitespace-nowrap sm:table-cell dark:text-neutral-300">
                                {{ $sale->created_at->format('d/m/Y') }}
                                <span class="text-xs text-neutral-400 dark:text-neutral-500">{{ $sale->created_at->format('H:i:s') }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3 font-semibold whitespace-nowrap text-neutral-800 dark:text-neutral-100">${{ number_format($sale->total, 2) }}</td>
                        </tr>
                        <tr data-sale-details="{{ $sale->id }}" class="hidden">
                                    ...
                                    <ul class="grid gap-2 lg:grid-cols-2">
                                        @foreach ($sale->details as $detail)
                                            <li class="flex items-center justify-between gap-3 rounded-lg border border-neutral-200/80 bg-white px-3 py-2 text-xs shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                                                <span class="min-w-0 truncate font-medium text-neutral-800 dark:text-neutral-200">{{ $detail->product?->name ?? 'Producto eliminado' }}</span>
                                                <span class="shrink-0 text-neutral-400 dark:text-neutral-500">{{ $detail->quantity }} × ${{ number_format($detail->unit_price, 2) }}</span>
                                                <span class="shrink-0 font-semibold text-neutral-800 dark:text-neutral-100">${{ number_format($detail->subtotal, 2) }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
