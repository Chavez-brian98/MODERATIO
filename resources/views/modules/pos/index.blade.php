        ...
        <div class="flex min-h-0 flex-1 flex-col">
            ...
            <div class="mt-3 flex flex-wrap gap-2" id="category-filters">
                ...
                @foreach ($categories as $cat)
                    <button type="button" data-category-filter="{{ $cat->id }}" class="inline-flex items-center rounded-full border border-neutral-200 bg-white px-3 py-1 text-xs font-semibold text-neutral-600 transition-colors hover:bg-neutral-100 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:bg-neutral-700">
                        {{ $cat->name }}
                    </button>
                @endforeach
            </div>
            ...
            <div id="product-grid" class="mt-4 grid grid-cols-2 gap-3 overflow-y-auto sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4" style="scrollbar-width: thin;">
                @forelse ($products as $product)
                    <button
                        ...
                        data-product-tax="{{ $product->has_tax ? $product->tax_percentage : 0 }}"
                        data-product-category="{{ $product->category_id }}"
                        data-product-search="{{ strtolower($product->name . ' ' . ($product->barcode ?? '') . ' ' . $product->category?->name) }}"
                    ...
                    >
                        ...
                        <p class="mt-2 truncate text-sm font-semibold text-neutral-900 dark:text-white" title="{{ $product->name }}">{{ $product->name }}</p>
                        <p class="mt-0.5 truncate text-xs text-neutral-500 dark:text-neutral-400">{{ $product->category?->name ?? 'Sin categoría' }}</p>
                        <div class="mt-auto flex items-end justify-between pt-2">
                            <p class="text-lg font-bold text-brand-700 dark:text-brand-400">${{ number_format($product->sale_price, 2) }}</p>
                            <span class="text-xs {{ $product->current_stock <= $product->min_stock ? 'font-semibold text-amber-600 dark:text-amber-400' : 'text-neutral-400 dark:text-neutral-500' }}">
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let cashRegisterOpen = @json($cashRegister !== null);
            ...
            customerSearch.addEventListener('input', () => {
                ...
                customerSearchTimeout = setTimeout(async () => {
                    try {
                        const resp = await fetch('{{ route("pos.customers") }}?q=' + encodeURIComponent(q), {
            ...
            document.getElementById('form-open-register').addEventListener('submit', async (e) => {
                ...
                try {
                    const resp = await fetch('{{ route("pos.cash-register.open") }}', {
                        method: 'POST',
                        headers: {
            ...
            btnPay.addEventListener('click', async () => {
                ...
                try {
                    const resp = await fetch('{{ route("pos.sale.complete") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
