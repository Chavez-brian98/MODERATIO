...
@section('content')
    ...
    <header class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <div class="hidden h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-400 sm:flex">
                <i class="fa-solid fa-tags text-lg" aria-hidden="true"></i>
            </div>
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-white">Nueva categoría</h1>
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Registra una categoría para clasificar tus productos.</p>
            </div>
        </div>
        <a
    ...
    <div class="mt-6 grid items-start gap-6 lg:grid-cols-3">
        <div class="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2">
            ...
            <form method="POST" action="{{ route('categories.store') }}" class="space-y-6 px-6 py-6 sm:px-8">
                @csrf
        ...
        <aside class="space-y-6">
            <div class="rounded-2xl border border-brand-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                ...
                <p class="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                    {{ $categories->count() }} {{ $categories->count() === 1 ? 'categoría registrada' : 'categorías registradas' }}.
                </p>

                @if ($categories->isNotEmpty())
                    <ul class="mt-4 space-y-2.5">
                        @foreach ($categories->take(5) as $option)
                            <li class="flex items-center justify-between gap-3 text-sm">
                                <span class="min-w-0 truncate text-neutral-700 dark:text-neutral-300">{{ $option->name }}</span>
                                <span class="shrink-0 rounded-full bg-brand-50 px-2 py-0.5 text-xs font-medium text-brand-700 dark:bg-brand-900/40 dark:text-brand-400">
                                    {{ $option->products_count }} {{ $option->products_count === 1 ? 'producto' : 'productos' }}
                                </span>
                            </li>
                        @endforeach
                    </ul>
                    @if ($categories->count() > 5)
                        <p class="mt-3 text-xs text-neutral-400 dark:text-neutral-500">Y {{ $categories->count() - 5 }} más...</p>
                    @endif
                @else
            ...
            <div class="rounded-2xl border border-brand-200 bg-brand-50/60 p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-800/40">
                ...
                <ul class="mt-4 space-y-3 text-sm text-neutral-600 dark:text-neutral-400">
                    <li class="flex gap-2.5">
                        ...
                        Usa nombres claros y cortos, ej. «Bebidas».
                    </li>
                    <li class="flex gap-2.5">
                        <i class="fa-solid fa-circle-check mt-0.5 text-brand-600 dark:text-brand-400" aria-hidden="true"></i>
                        Agrupa productos relacionados en una categoría padre.
                    </li>
                    <li class="flex gap-2.5">
                        <i class="fa-solid fa-circle-check mt-0.5 text-brand-600 dark:text-brand-400" aria-hidden="true"></i>
    ...
    <script>
        function parentSearch() {
            return {
                search: '',
                showDropdown: false,
                selectedId: '{{ old("parent_category_id") }}',
                categories: @json($categories->map(fn ($c) => ['id' => $c->id, 'name' => $c->name, 'products_count' => $c->products_count])),
                filteredCategories: [],
