...
@section('title', 'Editar categoría · ' . config('app.name'))
...
@section('content')
    ...
    <header class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Actualiza la información de {{ $category->name }}.</p>
            </div>
        </div>
        <a
    ...
    <div class="mt-6 grid items-start gap-6 lg:grid-cols-3">
        <div class="overflow-hidden rounded-2xl border border-brand-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2">
            ...
            <form method="POST" action="{{ route('categories.update', $category) }}" class="space-y-6 px-6 py-6 sm:px-8">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center justify-between gap-4 rounded-xl border border-brand-200 bg-brand-50/40 px-4 py-3.5 dark:border-neutral-800 dark:bg-neutral-800/40">
                    ...
                    <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $category->is_active)) class="peer sr-only" />
    ...
    <script>
        function parentSearch() {
            return {
                search: '',
                showDropdown: false,
                selectedId: '{{ old("parent_category_id", $category->parent_category_id) }}',
