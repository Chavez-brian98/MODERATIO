        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-800" role="grid">
                ...
                @forelse ($categories as $category)
                    <tr
                        data-category-row
                        data-category-search="{{ strtolower(trim($category->name . ' ' . ($category->parent?->name ?? '') . ' ' . ($category->description ?? ''))) }}"
                        class="{{ $category->is_active ? 'hover:bg-brand-50/40 dark:hover:bg-neutral-800/50' : 'bg-neutral-50 hover:bg-neutral-100/60 dark:bg-neutral-800/30 dark:hover:bg-neutral-800/60' }}"
                    >
                        <td class="whitespace-nowrap px-4 py-3 text-neutral-500 dark:text-neutral-400 sm:px-6">#{{ $category->id }}</td>
                        <td class="whitespace-nowrap px-4 py-3 sm:px-6">
                            <p class="{{ $category->is_active ? 'font-medium text-neutral-900 dark:text-white' : 'font-medium text-neutral-400 dark:text-neutral-500' }}">{{ $category->name }}</p>
                            @if ($category->description)
                                <p class="mt-0.5 max-w-xs truncate text-xs text-neutral-400 dark:text-neutral-500">{{ $category->description }}</p>
                            @endif
                        </td>
                            ...
                            <div class="flex items-center justify-end gap-1.5">
                                @if (Route::has('categories.show'))
                                    <button
                                        type="button"
                                        title="Ver detalle"
                                        data-view-category="{{ route('categories.show', $category) }}"
